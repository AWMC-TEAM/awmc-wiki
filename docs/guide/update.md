...
- **我的猜歌** / `猜歌数据` / `猜歌统计` — 个人五模式（猜歌 / 曲绘 / 曲子 / 铺面 / 开字母）近 30 日积分趋势、积分/次数双雷达、模式卡片与近期明细；猜对与开字母结算后实时写入
- 支持 `我的猜歌 @某人` / `猜歌数据 @某人` 查看对方本群数据（需群已开启猜歌）
...
- **结算展示** — 先短文案（用时 `xx.xxx秒` + 星级 + 奖池），再本局贡献分成图，最后全开看板；可查 `开字母排行`、`开字母贡献榜`、`开字母时间榜`
- `不玩了` 仅揭晓，不发速度奖与贡献奖
- **猜歌答题限流** — 同一用户全局 3 秒只能答一次（猜歌 / 曲绘 / 曲子 / 铺面 / 开字母）；超限提示「嘿嘿，你的答案被我吃掉啦！」
- **舞萌状态** / `mais` — 「服务器失败率」折线图（账号操作全量日志：发票、maiu 上传、绑定等；`returnCode=0` 计失败；无数据时段省略）+ 服务器实时状态
